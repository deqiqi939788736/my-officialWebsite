...
import '../../net/aesTool/aesHsahTool.dart';
// import '../../utils/get_toast.dart';
...
extension ControllerExt on GetxController {
  ...
  Future<void> asyncRequest(Future<Response> Function() request,
      ...
      bool isDecrypt = true}) async {
    try {
      Response response = await request();
      //加密字符串
      var ecDataString = response.data;
      var dataString;
      if (isDecrypt) {
        dataString = JhEncryptUtils.aesDecrypt(ecDataString);
        if (dataString == "") {
          debugPrint("解密失败");
          return;
        }
      } else {
        dataString = ecDataString;
      }
      Map<String, dynamic> dataMap;
      if (dataString is Map<String, dynamic>) {
        dataMap = dataString;
        onSuccess?.call(dataMap);
        return;
      }
      dataMap = jsonDecode(dataString);
      debugPrint("响应成功 \n" + dataString);
      var code = dataMap["status"];
      var msg = dataMap["message"];
      //业务码异常处理
      if (code == 0) {
        if (dataMap['token'] != null) {
          onSuccess?.call(dataMap['token']);
        } else {
          onSuccess?.call(dataMap['data']);
        }
      } else {
        onFailure?.call(dataMap, code, msg);
        if (msg != null && isShowErrorMsg) {
          // Loading.dismiss();
          // Loading.showError(msg);
          // GetToast.clearLoading();
          // GetToast.showText(msg ?? "未知错误");
        }
      }
